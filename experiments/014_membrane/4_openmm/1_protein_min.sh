...
INPUT_PDB="/scratch/users/k25053715/MSc_research/4_openmm_project/2_fgfr2_minimisation/012_finetune/012_FGFR2_clean_linkers.pdb"

cp "$INPUT_PDB" ./012_FGFR2_clean_linkers.pdb
echo "Copied: $INPUT_PDB → ./012_FGFR2_clean_linkers.pdb"

cat > 1_protein_min.py << 'PYEOF'
import openmm
import openmm.app
...
openmm.app.PDBFile.writeFile(fixer.topology, fixer.positions, open('014_1_clean_fixed.pdb', 'w'))

############################################
# Step 2: Forcefield + hydrogens
############################################

pdb = openmm.app.PDBFile('014_1_clean_fixed.pdb')
...
openmm.app.PDBFile.writeFile(modeller.topology, modeller.positions, open('014_2_clean_withH.pdb', 'w'))
...
openmm.app.PDBFile.writeFile(
    simulation.topology,
    positions,
    open('014_3_fgfr2_clean_minimized.pdb', 'w')
)
...
python 1_protein_min.py

echo "Results:"
ls -lh 014_*.pdb 012_FGFR2_clean_linkers.pdb
